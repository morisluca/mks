...
export default function DepositTransactions() {
  ...
  return (
    <UserLayout>
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Deposit Transactions</h1>
          <p className="text-sm text-muted-foreground mt-1">All your deposits</p>
        </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody className="divide-y divide-border">
                  {transactions?.filter(d => d.type === "deposit")?.map((tx) => (
                    <tr key={tx.id} className="hover:bg-secondary/50 transition-colors">
                      ...
                      <td className={`px-4 py-3 text-right text-sm font-semibold ${tx.type === "deposit" ? "text-green-400" : "text-red-400"}`}>
                        {tx.type === "deposit" ? "+" : "-"}{settings?.currency} {parseFloat(String(tx.amount ?? 0)).toFixed(2)}
                      </td>
